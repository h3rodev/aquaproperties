...
<?php 
    $pagetitle = $project->seo_title;
?>

@section('title', $pagetitle )

@section('banner-slider')
<div class="full-image-spacer" style="background:url('/{{ $project->featured_image }}')">
...
<section>
    ...
    @php
        $brochure = explode(',' , $project->brochure_url );
        $floor_plans = explode(',' , $project->floor_plans_url );
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="{{ $project->video_url }}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                </div>
            </div>
            <div class="col-md-6">    
                {!! $project->features !!}
...
@if($project->gallery != '')
    <section>
        <div class="container">
            <h3>Gallery</h3>
            <div class="row">
                @foreach( explode(',', str_replace('"', '',  str_replace(']', '', str_replace('[', '', $project->gallery ) ) ) ) as $gallery )
                    <a href="https://aquaproperties.app/{{  $gallery }}" data-toggle="lightbox" data-gallery="gallery" class="col-md-4 ligthbox-holder">
                        <img class="img-thumbnail" src="https://aquaproperties.app/{{  $gallery }}" alt="{{ $project->project_name }}">
                    </a>
                @endforeach
            </div>
        </div>
...
@if($project->floor_plans != '')
    <section>
        <div class="container">
            <h3>Floor Plans</h3>
            <div class="row">
                @foreach( explode(',', str_replace('"', '',  str_replace(']', '', str_replace('[', '', $project->floor_plans ) ) ) ) as $floor_plan )
                    <a href="https://aquaproperties.app/{{  $floor_plan }}" data-toggle="lightbox" data-gallery="floor-plans" class="col-md-4 ligthbox-holder">
                        <img class="img-thumbnail" src="https://aquaproperties.app/{{  $floor_plan }}" alt="{{ $project->project_name }}">
                    </a>
                @endforeach
            </div>
        </div>
            ...
            <div class="col-md-7">
                <div class="contact-us-holder">
                    <iframe
                        ...
                        src="https://www.google.com/maps/embed/v1/place?key=AIzaSyBsF0s-gHbwok2megHTkehrb5QWMw99z70&q={{ $project->location }}" allowfullscreen>
                    </iframe>
                </div>
            </div>
            <div class="col-md-5">
