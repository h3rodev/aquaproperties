...
<?php 
    $pagetitle = $area->seo_title ? $area->seo_title : $area->area_name .' community by '. $area->developer;
?>

@section('title', $pagetitle )

@section('banner-slider')
<div class="full-image-spacer" style="background:url('/{{ $area->featured_image }}')">
    <div class="container banner-content">

        <div class="row">
            <div class="col-md-7">
                <h1>{{ $area->area_name }} by {{ $area->developer }}</h1>
                <h2>{{ $area->sub_title }}</h2>
            </div>
            <div class="col-md-5">
...
<nav class="breadcrumb-holder">
    <div class="container">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="/off-plan-properties-dubai/{{ $area->slug }}">{{ $area->area_name }}</a></li>
        </ol>
    </div>
</nav>
...
<section>
    ...
    {!! $area->main_description !!}

    @php
        $brochure = explode(',' , $area->brochure_url );
        $floor_plans = explode(',' , $area->floor_plans_url );
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="{{ $area->video_url }}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                </div>
            </div>
            <div class="col-md-6">    
                {!! $area->features !!}
...
@if( count($offplans) > 0 )
    <section>
        <div class="container">
            <h3>Featured Off Plan Projects in {{ $area->area_name}} </h3>
            <div class="row">
                @foreach($offplans as $offplan)
                    <div class="col-md-4">
                        <div class="card mb-5">
                            <img class="card-img-top" src="/{{ $offplan->featured_image }}" alt="{{ $offplan->project_name }}<">
                            <div class="off-plan card-body">
                                
                                @if($offplan->sub_title)
                                    <h5 class="card-text">{{ $offplan->sub_title }}</h5>
                                @endif
                                <h6 class="card-title">{{ $offplan->project_name }}</h6>
                                <p class="card-text">by {{ $offplan->developer }}</p>
                                <a class="btn aqua-btn-outline-gray" href="/off-plan-properties-dubai/{{ $area->slug }}/{{ $offplan->slug }}" class="btn btn-primary">Read more</a>
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="col-md-7">
                <div class="contact-us-holder">
                    <iframe
                        ...
                        src="https://www.google.com/maps/embed/v1/place?key=AIzaSyBsF0s-gHbwok2megHTkehrb5QWMw99z70&q={{ $area->location }}" allowfullscreen>
                    </iframe>
                </div>
            </div>
            <div class="col-md-5">
